...
During Phase 2 I used ChatGPT as a learning and debugging tool. The AI helped me understand the required concepts and guided me while writing the implementation myself.

## remove_district

I used AI to better understand how the new `remove_district` command should work.

The discussions helped me understand:

- how `fork()` creates a child process
- how `execlp()` runs `rm -rf`
- how `waitpid()` waits for the child process
- how `unlink()` removes the symbolic link

After that, I wrote and integrated the function into my own project structure.

## monitor_reports

I also used AI to better understand signal handling and process monitoring.

The discussions helped me understand:

- how `.monitor_pid` should be created
- how to store the process ID using `getpid()`
- how `SIGUSR1` and `SIGINT` work
- how `sigaction()` connects a signal to a handler
- how `pause()` keeps the process waiting for signals

I then implemented the monitor program myself and tested it manually.

## What I learned

During Phase 2 I better understood:

- process creation and parent-child behavior
